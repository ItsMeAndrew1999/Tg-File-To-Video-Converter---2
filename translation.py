class Translation(object):
    START_TEXT = """Hello,
I am Telegram File-To-Video Converter Bot!
         Supporting Channel : @naveenfiletovideobot
Please send me any direct download URL Link, i can upload to telegram as File/Video

 🚨 . . . NB: its support almost all kind of video files like mp4,mkv..... except shortened videos & some formats . . . 🚨
 
🚨 PRON video🔞 gives you PERMANENT BAN 🚨

       ┈┈┈••💙✿❤️✿💚••┈┈┈
       
Bot created by @naveen_tn49"""
    ABS_TEXT = " Please don't be selfish."
    BANNED_USER_TEXT = "you are banned"
    UPGRADE_TEXT = "No upgrade plan 🥰"
    DOWNLOAD_START = "Downloading to my server 📥 Please wait...⏳ 🙇🙇🙇 it takes time depend on File Size"
    UPLOAD_START = "Yay,File Download Successfully 😊 Now Uploading to Telegram 📤"
    RCHD_BOT_API_LIMIT = "size greater than maximum allowed size (50MB). Neverthless, trying to upload."
    AFTER_SUCCESSFUL_UPLOAD_MSG = "Thanks for using me. Join my main Channel :- @naveenfiletovideobot"
    AFTER_SUCCESSFUL_UPLOAD_MSG_WITH_TS = "Downloaded in {} seconds.\nUploaded in {} seconds."
    NOT_AUTH_USER_TEXT = "Please /upgrade your subscription."
    SAVED_CUSTOM_THUMB_NAIL = "Custom video / file thumbnail saved. This image will be used in next 24 hr"
    DEL_ETED_CUSTOM_THUMB_NAIL = "✅ Custom thumbnail cleared succesfully. Join my Channel @naveenfiletovideobot."
    FF_MPEG_DEL_ETED_CUSTOM_MEDIA = "✅ Media cleared succesfully."
    SAVED_RECVD_DOC_FILE = "Document Downloaded Successfully."
    CUSTOM_CAPTION_UL_FILE = ""
    NO_CUSTOM_THUMB_NAIL_FOUND = "No Custom ThumbNail found."
    CURENT_PLAN_DETAILS = """Current plan details
--------
Telegram ID: <code>{}</code>
Plan name: "Premium Version"
Expires on: {}"""
    HELP_USER = """I can do:
👉 Reply /converttovideo to Convert To Streamable video any telegram file
--------

Send /me to know current plan details

"""

    REPLY_TO_DOC_FOR_C2V = "Reply to a Telegram media to convert"
